...
`ZWATCH` blocks the connection that issued it, so a connection watches at most one key at a time. A client
waits for the signal or error before sending its next command on that connection.
...
- A connection watches at most one key at a time, since the command blocks the connection until it fires or fails.
