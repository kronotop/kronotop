...
public class KrAdminCommandBuilder<K, V> extends BaseKronotopCommandBuilder<K, V> {
    ...
    @SuppressWarnings({"unchecked", "rawtypes"})
    public Command<K, V, Map<String, Object>> describeCluster() {
        CommandArgs<K, V> args = new CommandArgs<>(codec).add(CommandKeyword.DESCRIBE_CLUSTER);
        return createCommand(CommandType.KR_ADMIN, (MapOutput) new MapOutput<String, Object>((RedisCodec) codec), args);
    }
    ...
    public Command<K, V, V> dropCluster(String clusterName) {
        CommandArgs<K, V> args = new CommandArgs<>(codec).add(CommandKeyword.DROP_CLUSTER).add(clusterName);
        return createCommand(CommandType.KR_ADMIN, new ValueOutput<>(codec), args);
    }

    public Command<K, V, String> dropCluster(String clusterName, String token) {
        CommandArgs<K, V> args = new CommandArgs<>(codec).add(CommandKeyword.DROP_CLUSTER).add(clusterName).add(token);
        return createCommand(CommandType.KR_ADMIN, new StatusOutput<>(codec), args);
    }
