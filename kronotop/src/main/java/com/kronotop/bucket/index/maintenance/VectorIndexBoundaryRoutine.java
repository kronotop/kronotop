...
/**
 * Determines scan boundaries for vector index building and creates
 * {@link IndexBuildingTask}s for all shards.
 *
 * @see VectorIndexUtil
 * @see BoundaryLocator
 */
public class VectorIndexBoundaryRoutine extends AbstractBoundaryRoutine {

    public VectorIndexBoundaryRoutine(Context context,
    ...
    protected void saveDefinition(Transaction tr, BucketMetadata metadata, IndexDefinition definition) {
        VectorIndexUtil.saveIndexDefinition(tr, metadata, (VectorIndexDefinition) definition);
    }
    ...
    protected void createBuildingTasks(TransactionalContext tx, BucketMetadata metadata, Boundaries boundaries) {
        for (int shardId : metadata.shards()) {
            VectorIndexUtil.createVectorIndexBuildingTask(tx, metadata, task.getIndexId(), shardId, boundaries);
        }
    }
}
