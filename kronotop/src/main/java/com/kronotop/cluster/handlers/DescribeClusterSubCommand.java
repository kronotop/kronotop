...
class DescribeClusterSubCommand extends BaseSubCommand implements SubcommandHandler {
    ...
    DescribeClusterSubCommand(SimpleMembershipService service) {
        super(service);
    }
    ...
    private DirectorySubspace openShardSubspace(Transaction tr, DirectorySubspace subspace, ShardKind kind, int shardId) {
        return subspace.open(tr, List.of("shards", kind.toString().toLowerCase(), Integer.toString(shardId))).join();
    }
