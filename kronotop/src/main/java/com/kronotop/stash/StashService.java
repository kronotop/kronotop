...
import com.kronotop.stash.handlers.cluster.ClusterHandler;
import com.kronotop.stash.handlers.connection.SelectHandler;
import com.kronotop.stash.handlers.generic.*;
import com.kronotop.stash.handlers.hash.*;
import com.kronotop.stash.handlers.string.*;
import com.kronotop.stash.handlers.transactions.*;
import com.kronotop.stash.server.FlushAllHandler;
...
public class StashService extends ShardOwnerService<StashShard> implements KronotopService {
    ...
    private final ScheduledExecutorService scheduler = new ScheduledThreadPoolExecutor(
            ...
            new ThreadFactoryBuilder().setNameFormat("kr.stash-service-%d").build()
    );
    private final int numberOfShards;
    private final List<VolumeSyncWorker> volumeSyncWorkers = new ArrayList<>();

    public StashService(Context context) throws CommandAlreadyRegisteredException {
        super(context, NAME);

        this.watcher = context.getService(Watcher.NAME);
        this.numberOfShards = context.getConfig().getInt("stash.shards");
        ...
        handlerMethod(ServerKind.EXTERNAL, new ExecHandler());
        ...
        routing.registerHook(RoutingEventKind.LOAD_STASH_SHARD, new LoadStashShardHook());
    }
     ...
     *
     * @param container the StashValueContainer holding the Stash value whose kind is to be checked
     * @param kind      the expected StashValueKind that the value should be
     * @throws WrongTypeException if the value kind in the container does not match the expected kind
     */
    public static void checkStashValueKind(StashValueContainer container, StashValueKind kind) {
        if (container == null) {
            return;
        }
    ...
    private void initializeVolumeSyncerWorkers() {
        int numVolumeSyncWorkers = context.getConfig().getInt("stash.volume_syncer.workers");
        int period = context.getConfig().getInt("stash.volume_syncer.period");
    ...
    private void createAndLoadStashShard(int shardId) {
        getServiceContext().shards().put(shardId, new OnHeapStashShardImpl(context, shardId));
        StashShard shard = getServiceContext().shards().get(shardId);
        StashShardLoader loader = new StashShardLoader(context, shard);
        loader.load();
    }

    /**
     * Loads the Stash shard from the local disk based on the given shard ID.
     ...
     */
    private void loadStashShardFromDisk(int shardId) {
        ...
        if (route.primary().equals(context.getMember()) || route.standbys().contains(context.getMember())) {
            LOGGER.trace("Loading Stash Shard: {} from the local disk", shardId);
            // Primary or standby owner
            createAndLoadStashShard(shardId);
        }
    }

    public void start() {
        for (int shardId = 0; shardId < numberOfShards; shardId++) {
            loadStashShardFromDisk(shardId);
        }
     ...
     */
    private StashShard getShard(Integer shardId) {
    ...
    public void shutdown() {
        ...
        if (!ExecutorServiceUtil.shutdownNowThenAwaitTermination(scheduler)) {
            LOGGER.warn("Stash scheduled executor service could not be stopped gracefully");
        }
    }

    /**
     * Finds a Stash shard by its ID and checks if it matches the desired shard status.
     ...
     * @return the StashShard object if the shard is found and its status matches the desired status
     * @throws KronotopException if the shard is in an inoperable status or does not match the desired status
     */
    private StashShard findShardAndCheckStatus(Route route, int shardId, ShardStatus desiredShardStatus) {
    ...
    /**
     * Finds the route information for a Stash shard with the specified shard ID.
     *
     ...
     */
    private StashShard findShard_internal(int slot, ShardStatus desiredShardStatus) {
     ...
     */
    public StashShard findShard(String key, ShardStatus desiredShardStatus) {
     ...
     */
    public StashShard findShard(List<String> keys, ShardStatus desiredShardStatus) {
    ...
    /**
     * Finds and retrieves a Stash shard with a given shard ID and verifies its status.
     *
     ...
     * @return the StashShard object associated with the specified shard ID and desired status
     * @throws KronotopException if the shard is inoperable or if its status does not match
     *                           the desired status
     */
    public StashShard findShard(int shardId, ShardStatus desiredShardStatus) {
        Route route = findRoute(shardId);
        return findShardAndCheckStatus(route, shardId, desiredShardStatus);
    }
    ...
    public List<SlotRange> getSlotRanges() {
        ...
        for (int hashSlot = 0; hashSlot < NUM_HASH_SLOTS; hashSlot++) {
            ...
            if (currentShardId != null && shardId != currentShardId) {
                ...
                Route route = routing.findRoute(ShardKind.STASH, currentShardId);
                if (route == null) {
    ...
    /**
     * LoadStashShardHook is a private class that implements the RoutingEventHook interface.
     * It is responsible for handling the loading of Stash shards from disk when a routing event occurs.
     */
    private class LoadStashShardHook implements RoutingEventHook {

        @Override
        public void run(ShardKind shardKind, int shardId) {
            final int finalShardId = shardId;
            Thread.ofVirtual().name("kr.stash.load-stash-shard").start(() -> loadStashShardFromDisk(finalShardId));
        }
    }
}
