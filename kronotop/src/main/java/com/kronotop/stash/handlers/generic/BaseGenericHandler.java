...
import com.kronotop.stash.handlers.string.BaseStringHandler;
...
public class BaseGenericHandler extends BaseStringHandler {
    public BaseGenericHandler(StashService service) {
        super(service);
    }

    protected void wipeOutKey(StashShard shard, String key, StashValueContainer previous) {
        shard.index().remove(key);
        deleteByVersionstamp(shard, previous.baseStashValue().versionstamp());
    }

    /**
     * Sets an expiration timeout on a key in the Stash system. This method is used to apply
    ...
    protected long expireCommon(String key, long ttl, ExpireMessage.Option option) {
        ...
        lock.writeLock().lock();
        try {
            StashValueContainer current = shard.storage().get(key);
            if (current == null) {
            ...
            if (!current.kind().equals(StashValueKind.STRING)) {
