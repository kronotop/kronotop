...
import com.kronotop.stash.NoAvailableShardException;
import com.kronotop.stash.StashService;
import com.kronotop.stash.handlers.BaseHandler;
import com.kronotop.stash.handlers.generic.protocol.ScanMessage;
import com.kronotop.stash.storage.StashShard;
import com.kronotop.stash.storage.index.Projection;
import com.kronotop.stash.storage.index.impl.FlakeIdGenerator;
...
public class ScanHandler extends BaseHandler implements Handler {
    ...
    private int findHostedShardId(int initial) {
        for (int shardId = initial; shardId < service.getNumberOfShards(); shardId++) {
            StashShard shard = service.getServiceContext().shards().get(shardId);
    ...
    public void execute(Request request, Response response) {
        ...
        StashShard shard = service.findShard(shardId, ShardStatus.READONLY);
        ...
        if (projection.getCursor() == 0) {
            try {
                ...
                StashShard nextShard = service.findShard(nextShardId, ShardStatus.READONLY);
                if (nextShard != null) {
