...
public class BaseHashHandler extends BaseHandler {
    ...
    public BaseHashHandler(StashService service) {
        super(service);
    }

    protected void syncHashField(StashShard shard, String key, FieldValuePair fieldValuePair) {
        AppendHashFieldJob job = new AppendHashFieldJob(key, fieldValuePair.getField());
        shard.volumeSyncQueue().add(job);
    }

    protected void deleteByVersionstamp(StashShard shard, HashFieldValue hashField) {
