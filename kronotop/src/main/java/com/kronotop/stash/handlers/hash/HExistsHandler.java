...
import com.kronotop.stash.handlers.hash.protocol.HExistsMessage;
...
public class HExistsHandler extends BaseHandler implements Handler {
    public HExistsHandler(StashService service) {
        super(service);
    }
    ...
    public void execute(Request request, Response response) throws Exception {
        ...
        StashShard shard = service.findShard(hexistsMessage.getKey(), ShardStatus.READWRITE);
        ReadWriteLock lock = shard.striped().get(hexistsMessage.getKey());
        lock.readLock().lock();
        try {
            StashValueContainer container = shard.storage().get(hexistsMessage.getKey());
            if (container == null) {
