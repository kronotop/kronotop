...
import com.kronotop.stash.handlers.hash.protocol.HGetAllMessage;
...
public class HGetAllHandler extends BaseHandler implements Handler {
    ...
    public void execute(Request request, Response response) throws Exception {
        ...
        StashShard shard = service.findShard(hgetallMessage.getKey(), ShardStatus.READONLY);
        ReadWriteLock lock = shard.striped().get(hgetallMessage.getKey());
        lock.readLock().lock();
        try {
            StashValueContainer container = shard.storage().get(hgetallMessage.getKey());
            if (container == null) {
