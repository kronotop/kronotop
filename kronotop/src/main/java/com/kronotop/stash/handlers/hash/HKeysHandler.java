...
import com.kronotop.stash.handlers.hash.protocol.HKeysMessage;
...
public class HKeysHandler extends BaseHandler implements Handler {
    ...
    public void execute(Request request, Response response) throws Exception {
        ...
        StashShard shard = service.findShard(hkeysMessage.getKey(), ShardStatus.READONLY);
        ReadWriteLock lock = shard.striped().get(hkeysMessage.getKey());
        lock.readLock().lock();
        try {
            StashValueContainer container = shard.storage().get(hkeysMessage.getKey());
            if (container == null) {
