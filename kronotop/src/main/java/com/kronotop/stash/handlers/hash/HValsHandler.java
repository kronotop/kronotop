...
import com.kronotop.stash.handlers.hash.protocol.HValsMessage;
...
public class HValsHandler extends BaseHandler implements Handler {
    public HValsHandler(StashService service) {
        super(service);
    }
    ...
    public void execute(Request request, Response response) throws Exception {
        ...
        StashShard shard = service.findShard(hvalsMessage.getKey(), ShardStatus.READONLY);
        ReadWriteLock lock = shard.striped().get(hvalsMessage.getKey());
        lock.readLock().lock();
        try {
            StashValueContainer container = shard.storage().get(hvalsMessage.getKey());
            if (container == null) {
