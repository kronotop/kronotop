...
public class AppendHandler extends BaseStringHandler implements Handler {
    ...
    private StashValueContainer appendCommand(StashShard shard, AppendMessage message, AtomicInteger result) {
        StashValueContainer previous = shard.storage().get(message.getKey());
        if (previous == null) {
            StringValue newValue = new StringValue(message.getValue());
            shard.storage().put(message.getKey(), new StashValueContainer(newValue));
            result.set(newValue.value().length);
            return null;
        }

        if (!previous.kind().equals(StashValueKind.STRING)) {
    ...
    public void execute(Request request, Response response) {
        ...
        try {
            lock.writeLock().lock();
            StashValueContainer previous = appendCommand(shard, message, result);
