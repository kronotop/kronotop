...
/**
 * The StashShardLoader class is responsible for loading and processing data from a StashShard's
 * storage volume into its operational state. It interacts with a Context and StashShard to read,
 * process, and update data structures stored in the shard.
 */
public final class StashShardLoader {
    private static final Logger LOGGER = LoggerFactory.getLogger(StashShardLoader.class);
    private final Context context;
    private final StashShard shard;
    private final Prefix prefix;

    public StashShardLoader(Context context, StashShard shard) {
        this.context = context;
        this.shard = shard;
        this.prefix = new Prefix(context.getConfig().getString("stash.volume_syncer.prefix").getBytes());
    }
     ...
     */
    public static void processStringPack(StashShard shard, VolumeEntry entry) throws IOException {
        StringPack pack = StringPack.unpack(entry.entry());
        ReadWriteLock lock = shard.striped().get(pack.key());
        lock.writeLock().lock();
        try {
            StashValueContainer container = new StashValueContainer(pack.stringValue());
     ...
     * @throws IOException if the KeyEntry cannot be unpacked or contains invalid data
     */
    public static void processHashFieldPack(StashShard shard, VolumeEntry entry) throws IOException {
        HashFieldPack pack = HashFieldPack.unpack(entry.entry());
        ReadWriteLock lock = shard.striped().get(pack.key());
        lock.writeLock().lock();
        try {
            StashValueContainer container = shard.storage().get(pack.key());
            if (container == null) {
                container = new StashValueContainer(new HashValue());
                shard.storage().put(pack.key(), container);
            }
