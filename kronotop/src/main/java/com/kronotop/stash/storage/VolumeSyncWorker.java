...
/**
 * The VolumeSyncWorker class is responsible for synchronizing volume in Stash shards.
...
public class VolumeSyncWorker implements Runnable {
    ...
    private final ServiceContext<StashShard> stashContext;
    private volatile boolean shutdown = false;

    public VolumeSyncWorker(Context context, int workerId) {
        this.workerId = workerId;
        this.context = context;
        this.stashContext = context.getServiceContext(StashService.NAME);
        this.numWorkers = context.getConfig().getInt("stash.volume_syncer.workers");
    }
    ...
    /**
     * Executes the volume synchronization operation for the provided Stash shard.
     * <p>
     ...
     * </p>
     *
     * @param shard the Stash shard for which to execute the volume synchronization
     */
    private void runVolumeSyncer(StashShard shard) {
        ...
        try {
            ...
            LOGGER.error("Error while running volume syncer for Stash shard: {}", shard.id(), e);
        }
    }
    ...
    public void run() {
        try {
            ...
            try {
                stashContext.shards().forEach((shardId, shard) -> {
                ...
                LOGGER.error("Error while running Stash shard maintenance worker", e);
                throw e;
            }
        } catch (InterruptedException exp) {
            LOGGER.debug("Stash shard maintenance worker: {} is interrupted while running", workerId, exp);
            Thread.currentThread().interrupt();
            throw new KronotopException(exp);
        } finally {
            semaphore.release();
    ...
    private void drainVolumeSyncQueue(StashShard shard) {
        if (shard.volumeSyncQueue().isEmpty()) {
            return;
        }
        LOGGER.info("Draining volume sync queue on Stash shard: {}", shard.id());
    ...
    /**
     * Drains the volume synchronization queues for the Stash shards assigned to this worker.
     * <p>
    ...
    public void drainVolumeSyncQueues() {
        try {
            ...
            LOGGER.debug("Stash shard maintenance worker: {} is interrupted while draining the volume sync queues", workerId, exp);
            Thread.currentThread().interrupt();
            throw new KronotopException(exp);
        } finally {
            semaphore.release();
    ...
    public void shutdown() {
        try {
            ...
            LOGGER.debug("Failed to shutdown Stash shard maintenance worker", exp);
            Thread.currentThread().interrupt();
            throw new KronotopException(exp);
        } finally {
            semaphore.release();
