...
import com.kronotop.zmap.handlers.protocol.RangeKeySelector;
import com.kronotop.zmap.handlers.protocol.ZGetKeyMessage;
...
public class ZGetKeyHandler extends BaseZMapHandler implements Handler {
    ...
    public void execute(Request request, Response response) throws ExecutionException, InterruptedException {
        AsyncCommandExecutor.supplyAsync(context, response, () -> {
            ...
            CompletableFuture<byte[]> future = getKey(tr, keySelector, TransactionUtil.isSnapshotRead(session));
