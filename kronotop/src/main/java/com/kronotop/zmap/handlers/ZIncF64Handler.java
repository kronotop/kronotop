...
import com.kronotop.zmap.handlers.protocol.ZIncF64Message;
...
public class ZIncF64Handler extends BaseZMapHandler implements Handler {
    public ZIncF64Handler(ZMapService service) {
        super(service);
    }
    ...
    public void execute(Request request, Response response) throws Exception {
        runAsync(context, response, () -> {
            ...
            double value = 0.0;
            if (raw != null) {
                value = ZMapNumericValueCodec.decodeF64(raw);
            }
            double next = value + delta;
            ZMapNumericValueCodec.validateFiniteF64(next, "Resulting value is not a finite IEEE-754 double (overflow or invalid operation)");

            tr.set(key, ZMapNumericValueCodec.encodeF64(next));
