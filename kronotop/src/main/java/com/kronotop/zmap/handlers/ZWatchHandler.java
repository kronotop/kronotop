...
public class ZWatchHandler extends BaseZMapHandler implements Handler {
    ...
    public void execute(Request request, Response response) {
        ...
        runAsync(context, response, () -> {
            ...
            // ZWATCH blocks the connection, so a session holds at most one watched key at a time. Record
            // it before registering so a disconnect can release this client's demand on it; the slot is
            // cleared on every completion path below.
            session.attr(SessionAttributes.ZWATCH_KEY).set(packedKey);
            try {
                ...
                session.attr(SessionAttributes.ZWATCH_KEY).set(null);
            }
        }, response::writeOK);
    }
}
