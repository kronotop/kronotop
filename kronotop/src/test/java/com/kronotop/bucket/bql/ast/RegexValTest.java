...
class RegexValTest {
    ...
    @Test
    void shouldThrowOnBackreferencePattern() {
        // Behavior: RE2 does not support backreferences, so a backreference pattern is rejected at construction time.
        assertThrows(BqlParseException.class, () -> new RegexVal("(a)\\1", ""));
    }

    @Test
    void shouldThrowOnLookaroundPattern() {
        // Behavior: RE2 does not support lookaround, so a lookahead pattern is rejected at construction time.
        assertThrows(BqlParseException.class, () -> new RegexVal("foo(?=bar)", ""));
    }
