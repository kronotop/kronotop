...
public class DropSchemaTest extends BaseHandlerTest {
    @Test
    ...
    public void test_executeSuccessFully() {
        ...
        {
            ByteBuf buf = Unpooled.buffer();
            cmd.sql(SqlArgs.Builder.query("DROP SCHEMA foobar")).encode(buf);
            channel.writeInbound(buf);
            Object response = channel.readOutbound();
    ...
    @Test
    public void test_executeSchemaNotExists() {
        KronotopCommandBuilder<String, String> cmd = new KronotopCommandBuilder<>(StringCodec.ASCII);
        ...
        assertEquals("SQL Schema 'foobar' not exists", message.content());
    }
    ...
    public void test_executeIfExists() {
        ...
        ByteBuf buf = Unpooled.buffer();
        cmd.sql(SqlArgs.Builder.query("DROP SCHEMA IF EXISTS foobar")).encode(buf);
        channel.writeInbound(buf);
        Object response = channel.readOutbound();
