...
import com.kronotop.stash.storage.HashFieldPack;
...
public class BaseVolumeSyncIntegrationTest extends BaseStashHandlerTest {
    protected static final String key = "key";
    protected static final String value = "value";

    protected boolean volumeContainsStringKey(String key) {
        StashService service = instance.getContext().getService(StashService.NAME);
        StashShard shard = service.findShard(key, ShardStatus.READONLY);
    ...
    protected boolean volumeContainsHashField(String hashKey, String field) {
        StashService service = instance.getContext().getService(StashService.NAME);
        StashShard shard = service.findShard(hashKey, ShardStatus.READONLY);
        try (Transaction tr = service.getContext().getFoundationDB().createTransaction()) {
            VolumeSession session = new VolumeSession(tr, stashVolumeSyncerPrefix);
            // TODO: This can be done without a loop
            Iterable<VolumeEntry> iterable = shard.volume().getRange(session);
