...
public class ShardTest extends BaseStorageTest {
    protected StashShard shard;

    @BeforeEach
    public void beforeEach() {
        shard = new OnHeapStashShardImpl(context, 0);
    }
    ...
    public void test_put() {
        shard.storage().put("foo", new StashValueContainer(new StringValue("bar".getBytes())));
        makeAllShardsReadOnly();
        ShardReadOnlyException expected = assertThrows(ShardReadOnlyException.class, () -> shard.storage().put("boo", new StashValueContainer(new StringValue("foo".getBytes()))));
    ...
    public void test_computeIfAbsent() {
        ...
        ShardReadOnlyException expected = assertThrows(
                ShardReadOnlyException.class,
                () -> shard.storage().computeIfAbsent("boo", (key) -> new StashValueContainer(new StringValue("foo".getBytes())))
        );
        assertNotNull(expected);
    }
}
