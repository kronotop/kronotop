...
import com.kronotop.stash.storage.StringPack;
...
class VolumeSyncerIntegrationTest extends BaseNetworkedVolumeIntegrationTest {
    ...
    private void setStandby(KronotopTestInstance standbyInstance) {
        ...
        cmd.route("SET", RouteKind.STANDBY.name(), ShardKind.STASH.name(), standbyInstance.getMember().getId()).encode(buf);
    ...
    void sync_replication_when_standby_members_exists() throws IOException {
        ...
        await().atMost(Duration.ofSeconds(5)).until(() -> {
            int numberOfShards = kronotopInstance.getContext().getConfig().getInt("stash.shards");
        ...
        try (Transaction tr = context.getFoundationDB().createTransaction()) {
            ...
            Iterable<VolumeEntry> iterable = standbyVolume.getRange(session);
