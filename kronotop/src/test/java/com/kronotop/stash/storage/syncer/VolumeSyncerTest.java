...
package com.kronotop.stash.storage.syncer;
...
class VolumeSyncerTest extends BaseStorageTest {
    ...
    void shouldProcessMultipleJobsInSingleBatch() throws IOException {
        ...
        for (int i = 0; i < numEntries; i++) {
            ...
            shard.storage().put(key, new StashValueContainer(new StringValue(value.getBytes(), 0L)));
    ...
    void shouldHandleMixedAppendAndDeleteOperations() throws IOException {
        ...
        shard.storage().put(keyToDelete, new StashValueContainer(new StringValue("value-to-delete".getBytes(), 0L)));
        ...
        shard.storage().put(keyToKeep, new StashValueContainer(new StringValue("value-to-keep".getBytes(), 0L)));
    ...
    void shouldReturnVersionstampsAfterSync() {
        ...
        for (int i = 0; i < numEntries; i++) {
            String key = "key-" + i;
            shard.storage().put(key, new StashValueContainer(new StringValue(("value-" + i).getBytes(), 0L)));
            shard.volumeSyncQueue().add(new AppendStringJob(key));
        }
    ...
    void shouldSyncStringValue() throws IOException {
        ...
        shard.storage().put(expectedKey, new StashValueContainer(new StringValue(expectedValue.getBytes(), 0L)));
    ...
    void shouldSyncHashField() throws IOException {
        ...
        shard.storage().put(expectedKey, new StashValueContainer(hashValue));
